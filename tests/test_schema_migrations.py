...
@pytest.mark.asyncio
async def test_chunk_store_migrates_old_rows_and_round_trips_scope_fields(tmp_path) -> None:
    ...
    try:
        conn.executescript(
            ...
            INSERT INTO chunks VALUES (
                'chunk_old', 'doc_old', 'old schema text', 1, 1, 0, '/tmp/old.txt', 'hash-old',
    ...
    old_rows = await store.fetch_by_ids(["chunk_old"], tenant_id="default", owner_type="user", owner_id="user:u1")
    assert len(old_rows) == 1
    assert old_rows[0].tenant_id == "default"
    assert old_rows[0].workspace_id is None
...
@pytest.mark.asyncio
async def test_semantic_store_migrates_old_rows_and_persists_new_scope_columns(tmp_path) -> None:
    ...
    old_rows = await store.list_facts_for_owner(tenant_id="default", owner_type="user", owner_id="user:u1", limit=None)
    assert len(old_rows) == 1
    assert old_rows[0].tenant_id == "default"
    assert old_rows[0].session_id is None
    assert old_rows[0].meta["kind"] == "semantic_fact"
    assert old_rows[0].meta["kb_lane"] == "semantic"
...
@pytest.mark.asyncio
async def test_episodic_store_migrates_old_rows_and_persists_session_scope_fields(tmp_path) -> None:
    ...
    old_rows = await store.list_episodes(tenant_id="default", owner_type="user", owner_id="user:u1")
    assert len(old_rows) == 1
    assert old_rows[0].tenant_id == "default"
    assert old_rows[0].session_id is None
    assert old_rows[0].meta["kind"] == "episodic_event"
    assert old_rows[0].meta["kb_lane"] == "episodic"
...
@pytest.mark.asyncio
async def test_procedural_store_migrates_old_rows_and_persists_provenance_fields(tmp_path) -> None:
    ...
    old_rows = await store.list_skills(tenant_id="default", owner_type="user", owner_id="user:u1")
    assert len(old_rows) == 1
    assert old_rows[0].tenant_id == "default"
    assert old_rows[0].workspace_id is None
    assert old_rows[0].meta["kind"] == "procedural_rule"
    assert old_rows[0].meta["kb_lane"] == "procedural"
