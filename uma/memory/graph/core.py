...
class GraphCore:
    ...
    def __init__(self, adapter: GraphAdapter):
        ...
        if not isinstance(adapter, GraphAdapter):
            raise TypeError(
                f"GraphCore requires a GraphAdapter instance, "
        ...
        logger.info(
            "GraphCore initialized with adapter=%s",
    ...
    async def add_facts_batch(self, facts: list[Any]) -> int:
        ...
        try:
            ...
            logger.exception("GraphCore.add_facts_batch failed.")
            return 0
    ...
    def insert_fact_triplet(
        ...
        try:
            ...
            if owner_type == "system":
                raise ValueError("GraphCore.insert_fact_triplet: system owner_type is not supported")
            if owner_type == "user":
                owner_id = normalize_user_id(owner_id)
            if owner_type not in {"agent", "user", "workspace"}:
                raise ValueError(f"GraphCore.insert_fact_triplet: unsupported owner_type={owner_type!r}")
    ...
    def link_episode_to_facts(self, episode: Any, facts: list[Any]) -> None:
        ...
        try:
            ...
            logger.exception("GraphCore.link_episode_to_facts failed.")
    ...
    def link_temporal(self, prev_ep: Any, next_ep: Any) -> None:
        ...
        try:
            ...
            logger.exception("GraphCore.link_temporal failed.")
    ...
    def neighbors(
        ...
        if not user_id or not node_id:
            logger.warning("GraphCore.neighbors: missing user_id or node_id")
            return []

        if not owner_type or not owner_id:
            logger.error("GraphCore.neighbors: owner_type and owner_id are required")
            return []
        if not tenant_id:
            logger.error("GraphCore.neighbors: tenant_id is required")
            return []
        ...
        try:
            ...
            owner_type = validate_owner_type(owner_type)
            if owner_type == "system":
                raise ValueError("GraphCore.neighbors: system owner_type is not supported")
            if owner_type not in {"agent", "user", "workspace"}:
                raise ValueError(f"GraphCore.neighbors: unsupported owner_type={owner_type!r}")
            if owner_type == "user":
                owner_id = normalize_user_id(owner_id)
        except Exception:
            logger.exception("GraphCore.neighbors: invalid scoped graph request")
            return []
        ...
        try:
            logger.debug(
                "GraphCore.neighbors: node_id=%s owner=%s:%s depth=%d limit=%d preds=%s",
                node_id,
                owner_type,
                owner_id,
                depth_i,
            ...
            if not results:
                logger.warning(
                    "GraphCore.neighbors: returned 0 owner=%s:%s node_id=%s",
                    owner_type,
                    owner_id,
                    node_id,
                )
            ...
            logger.warning(
                "GraphCore.neighbors failed owner=%s:%s node_id=%s",
                owner_type,
                owner_id,
                node_id,
            )
            logger.exception("GraphCore.neighbors failed.")
            return []
    ...
    def resolve_nodes(
        ...
        *,
        ...
        try:
            ...
            owner_type = validate_owner_type(owner_type)
            if owner_type == "system":
                raise ValueError("GraphCore.resolve_nodes: system owner_type is not supported")
            if owner_type not in {"agent", "user", "workspace"}:
                raise ValueError(f"GraphCore.resolve_nodes: unsupported owner_type={owner_type!r}")
            if owner_type == "user":
                owner_id = normalize_user_id(owner_id)
        except Exception:
            logger.exception("GraphCore.resolve_nodes: invalid scoped graph request")
            return []
        ...
        for row in rows or []:
            try:
                ...
                logger.debug("graph: node ID extraction failed for row, skipping", exc_info=True)
                continue
        return out
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.exception("GraphCore.close failed.")
