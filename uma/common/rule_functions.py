...
Copied from the Animus WAF source (May 2026) with edits:
  - Replaced src.shared.logger with standard logging.
  - Removed print statements.
  - Removed entropy_score (was unused).
  - All regex patterns compiled once at module load via uma.common._regex_backend
    (prefers RE2 for ReDoS-proof execution; falls back to `re`). Previously the
    scorers recompiled their patterns on every invocation.
...
# ---------------------------------------------------------------------------
# Precompiled patterns — every regex used by the scorers below is compiled
# exactly once at module import. Keeps the write-time hot path free of
# per-call `re.compile` work and localises the backend choice.
# ---------------------------------------------------------------------------

_INTENT_VERB_NOUN_PAIRS = [
    ('bypass', 'moderation'),
    ('override', 'rules'),
    ('disable', 'filter'),
    ('print', 'context'),
...
_INTENT_VERB_NOUN_PATTERNS = tuple(
    _regex_backend.compile(rf'\b{verb}\b(?:\W+\w+){{0,5}}\W+\b{noun}\b')
    for verb, noun in _INTENT_VERB_NOUN_PAIRS
)

_INTENT_IMPERATIVE_PATTERNS = tuple(
    _regex_backend.compile(rf'^\b{v}\b')
    for v in ('ignore', 'override', 'disable', 'print', 'reveal')
)

_STRUCTURE_PATTERNS = tuple(
    _regex_backend.compile(pat, _regex_backend.IGNORECASE)
    for pat in (
        r'\[System\]:',
        r'^>>>\s*System',
        r'^###\s*System',
...
_ENCODING_PATTERNS = tuple(
    _regex_backend.compile(pat)
    for pat in (
        r'\b[0-9a-fA-F]{8,}\b',
...
def intent_score(prompt: str) -> int:
    ...
    score = 0
    for rx in _INTENT_VERB_NOUN_PATTERNS:
        if rx.search(prompt):
            score += 1

    stripped = prompt.strip()
    for rx in _INTENT_IMPERATIVE_PATTERNS:
...
def structure_score(prompt: str) -> int:
    ...
    return sum(1 for rx in _STRUCTURE_PATTERNS if rx.search(prompt))


def encoding_score(prompt: str) -> int:
    """Detects obfuscated or encoded text like hex or base64 payloads."""
    return sum(1 for rx in _ENCODING_PATTERNS if rx.search(prompt))
...
def persona_score(prompt: str) -> int:
    ...
    return sum(1 for alias in jailbreak_aliases if alias in lowered)


def cumulative_soft_triggers(prompt: str) -> int:
    lowered = prompt.lower()
    cs = sum(1 for c in _SOFT_TRIGGER_LITERALS if c in lowered)
    cp = sum(1 for rx in _SOFT_TRIGGER_PATTERNS if rx.search(prompt))
    return cs + cp
