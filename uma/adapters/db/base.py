"""
Database adapter abstractions for UMA.

This module defines a minimal but extensible database abstraction that
decouples UMA's stores from a specific DB backend such as SQLite or
PostgreSQL.
...
- Concrete implementations (e.g. SQLiteAdapter, PostgresAdapter) live in
  `uma.adapters.db.*` and implement DBAdapter.
...
- When adding new backends (PostgreSQL, DuckDB, etc.), create a new
  module under `uma.adapters.db` that subclasses DBAdapter and fully
...
class DBConnection(Protocol):
    ...
    functionality required by UMA stores.
    ...
    UMA stores explicitly manage commit/close in try/finally blocks
...
class DBAdapter(ABC):
    """
    Abstract base class for UMA database adapters.
    ...
    - Do not cache connections inside DBAdapter unless you also implement
      safe pooling behavior; UMA stores assume each call to
