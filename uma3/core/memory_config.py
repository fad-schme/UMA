...
class UMA3Config(dict):
    """Strict config loader with dot-access."""

    @classmethod
    def load_yaml(cls, path: str) -> "UMA3Config":
        try:
    ...
    def __getattr__(self, item: str):
        ...
        val = self[item]
        return UMA3Config(val) if isinstance(val, dict) else val
    ...
    def _validate(self):
        ...
        # SECURITY
        # -----------------------
        # Optional top-level security toggle that gates execution of code
        # embedded in configuration (disabled by default)
        if "security" in self:
            sec = self.security
            if not isinstance(sec, dict):
                raise ValueError("'security' section must be a mapping")
            if "allow_config_code" in sec and not isinstance(sec.get("allow_config_code"), bool):
                raise ValueError("'security.allow_config_code' must be boolean")
        # (optional tunables validated above)
